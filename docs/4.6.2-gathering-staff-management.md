...
- `member_id` → `members.id` (NO ACTION on delete)
...
- **Member/SCA Name (Autocomplete)** - Single field that handles both:
  ...
  - Based on `autoCompleteControl` element with `allowOtherValues = true`
...
User Input → Autocomplete Component
    ↓
Option A: AMP Member Selected
    member_public_id = "abc123"
    member_sca_name = "Jane of Example"
    ↓ (if steward)
    AJAX → getMemberContactInfo()
    ↓
    Auto-fill email/phone
    
Option B: Custom Name Entered
    member_public_id = (empty)
    member_sca_name = "John the Unknown"
    ↓
    Controller Transform
    member_sca_name → sca_name (DB column)
```
