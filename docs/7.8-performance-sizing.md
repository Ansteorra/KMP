...
When enabled, request timing telemetry is emitted to `logs/performance.log` with scope `app.performance` and SQL timing details are emitted through the `cake.database.queries` scope.
