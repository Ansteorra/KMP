...
**Features:**
- Files stored in `app/images/uploaded/`
- Direct access to files (no API required)
- Best for development and single-server deployments
- Fast access but not distributed
...
You can omit `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` when running on AWS
with an IAM role (EC2 instance profile, ECS task role, etc.).
...
- Highly available (geo-redundant options)
- Automatic backups
- Access control integration with Azure AD
...
- [Environment Variables](8.1-environment-setup.md) - Configuration reference
