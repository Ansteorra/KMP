...
- **Conditional Logic**: `{{#if}}` blocks allow templates to show or hide content based on variable values — no PHP execution, safe for admin editing
...
Database-stored templates support conditional blocks using a mustache-like `{{#if}}` syntax. This lets templates show or hide content based on variable values without any PHP execution — the syntax is parsed as a safe DSL using regex and string comparison.
...
| `{{#if a == "x" \|\| b == "y"}}...{{/if}}` | OR — show content when either condition is true |
| `{{#if a == "x" && b == "y"}}...{{/if}}` | AND — show content when both conditions are true |
...
**Important Notes:**
- No `eval()` or PHP execution — this is a safe pattern language using regex
