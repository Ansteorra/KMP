...
No seed command is required. The `KMP.DefaultTimezone` AppSetting is created
automatically (defaulting to `America/Chicago`) by
`App\Services\TenantDefaultSettingsInitializer` during application startup and
tenant provisioning.
