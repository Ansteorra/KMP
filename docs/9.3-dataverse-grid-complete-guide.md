...
$result = $this->processDataverseGrid([
    ...
    'queryCallback' => null,          // Presentation/context query callback
]);
```
...
- Locked system-view values are enforced server-side while that view is active
- Saved copies mark each locked filter in their own config; later updates preserve
  those canonical values even if a request tries to change or omit them
...
A column can declare `lockedFilter => true` when every use of that filter must
be locked. Set `showInFilterMenu => false` for system-only filters that users
must see in the toolbar but cannot add themselves. These filters still use the
normal pill appearance and are serialized when a custom view is saved:

```php
'warrant_scope' => [
    'key' => 'warrant_scope',
    'label' => __('Warrant state'),
    'filterable' => true,
    'filterType' => 'dropdown',
    'filterOnly' => true,
    'lockedFilter' => true,
    ...
    'exportable' => false,
    'filterOptions' => [
        ['value' => 'current', 'label' => __('Currently effective')],
        ['value' => 'previous', 'label' => __('Expired or inactive')],
    ],
    'customFilterHandler' => [
        'class' => WarrantsGridColumns::class,
...
Expression trees are appropriate when every condition maps directly to an
ordinary filter the user can see. If a view's meaning is dynamic or cannot be
faithfully represented by those individual conditions, define a symbolic,
filter-only column instead. Its custom handler can expand the saved value into
the required OR/AND query at request time:
...
**Key Points**:
- The active `warrant_scope=previous` value is visible as a locked toolbar pill.
- The symbolic value is ordinary saved-view data, so Save As and copy preserve it.
- `showInFilterMenu => false` prevents users from adding the system-only predicate.
- The handler resolves moving dates, the signed-in member, or other runtime data
  when the query runs; never serialize a stale computed date or identity.
- Invalid symbolic values should fail closed, and handlers require focused tests.
...
Every condition that defines a system view must be represented in `filters`,
including dynamic conditions such as “my queue,” “current month,” or “previous.”
Use a symbolic filter-only column with a custom handler when the condition cannot
be expressed as a normal field comparison. Because the condition remains regular
filter state, creating or copying a custom view preserves it without a separate
base-system-view relationship.
...
$result = $this->processDataverseGrid([
    ...
    'queryCallback' => function($query, $systemView) {
        // Add presentation-only selects/associations if the rendered view needs them.
        // Defining record scope belongs in serializable filters, not this callback.
        return $query;
    },
    // ...
]);
...
Do not put a system view's defining `where()` conditions in `queryCallback`.
Callbacks cannot be represented in saved configuration and make copied views
ambiguous. Use explicit filters, or a locked symbolic filter with a custom
handler, so the toolbar shows the complete record scope. Query callbacks are
reserved for presentation/context work that does not define the view, such as
loading an association required by a renderer. Authorization scope remains on
the base query and is intentionally not user-visible or copyable.
...
- Creating a custom view copies every active filter, including locked symbolic
  system filters, together with its presentation and user filters
- Locked filters use the same toolbar pill appearance as other filters, with a
  lock icon and accessible text in place of the remove control
...
Expression trees remain available for internal query composition, but they do
not serialize into the toolbar's active filter state. Do not use an expression
as the hidden definition of a system view that users can copy. Represent that
scope with a symbolic filter-only column and expand it in a custom handler:

```php
'config' => [
    'filters' => [
        ['field' => 'warrant_scope', 'operator' => 'eq', 'value' => 'current'],
    ],
],
```

This makes the full system-view meaning visible and ensures Save As/copy carries
it forward. Reserve standalone expressions for non-view internals where there is
no user-facing saved configuration to preserve.
