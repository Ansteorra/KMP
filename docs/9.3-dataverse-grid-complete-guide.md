...
| `filterOptionsSource` | string | - | Source for dynamic filter options (e.g., `'branches'`) |
...
'branch_id' => [
    ...
    'filterable' => true,
    'filterType' => 'dropdown',
    'filterOptionsSource' => 'branches',  // Dynamic options
    'defaultVisible' => true,
    'width' => '150px',
    'alignment' => 'left',
...
'branch_id' => [
    ...
    'filterable' => true,
    'filterType' => 'dropdown',
    'filterOptionsSource' => 'branches',  // Loads from loadFilterOptions()
],
```

Implement `loadFilterOptions()` in the trait (already provided for `'branches'`).
...
Extend `loadFilterOptions()` in your controller:
...
protected function loadFilterOptions(string $source): array
{
    switch ($source) {
        case 'my_custom_source':
            return $this->fetchTable('MyTable')
                ->find('list', keyField: 'id', valueField: 'name')
                ->map(fn($id, $name) => ['value' => (string)$id, 'label' => $name])
                ->toArray();
        default:
