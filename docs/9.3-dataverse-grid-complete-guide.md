...
- query/display mapping: `queryField`, `renderField`, `filterQueryField`;
  live dropdowns and saved filters (including expression trees) prefer `filterQueryField`
  for ID matching, then `queryField`, then the qualified column key. Keep this mapping
  identical across full-grid loads, table-frame refreshes, and CSV exports;
