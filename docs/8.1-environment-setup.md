...
| Variable | Type | Purpose | Example |
|----------|------|---------|---------|
| `DOCUMENTS_STORAGE_ADAPTER` | string | Storage backend | `local` or `azure` |
| `AZURE_STORAGE_CONNECTION_STRING` | string | Azure Blob Storage credentials | (when using Azure) |

See [Document Management System](4.7-document-management-system.md) for detailed storage configuration.
...
KMP loads configuration in this order:

1. `.env` - Environment variables
2. `config/app.php` - Baseline configuration with env() calls
3. `config/app_local.php` - Environment-specific overrides (if present)
4. Runtime configuration changes

Later values override earlier ones, allowing `.env` variables to override defaults.
