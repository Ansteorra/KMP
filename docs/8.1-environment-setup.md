---
layout: default
---
[← Back to Table of Contents](index.md) | [← Back to Deployment](8-deployment.md)

# 8.1 Environment Setup and Variables

This document provides comprehensive reference for environment variables used in KMP deployment and configuration.

## Overview

Environment variables control deployment-specific configuration without modifying code. They are loaded from `.env` files and support multiple deployment environments (development, staging, production) with a single codebase.

**Important:** Never commit `.env` files to version control. Each environment maintains its own `.env` file with sensitive credentials.

## Environment Variable Categories

Environment variables in KMP are organized by function:

- **Application**: Debug mode, titles, encoding
- **Database**: Connection strings and credentials
- **Security**: Cryptographic salts and tokens
- **Email**: Transport configuration and sender settings
- **Caching**: Cache backend configuration
- **Logging**: Log destination and service URLs
- **Timezone**: Default timezone for date/time operations
- **Document Storage**: Cloud storage credentials (if using Azure or S3)

## Core Variables Reference

### Application Configuration

| Variable | Type | Purpose | Example |
|----------|------|---------|---------|
| `DEBUG` | boolean | Enable debug mode and error display | `false` |
| `KMP_SESSION_COOKIE_SECURE` | boolean | Require HTTPS for session cookies; set `false` only for local HTTP development | `false` for local/test, otherwise `true` |
| `APP_ENCODING` | string | Character encoding | `UTF-8` |
| `APP_DEFAULT_LOCALE` | string | Locale for internationalization | `en_US` |
| `APP_DEFAULT_TIMEZONE` | string | Default timezone | `UTC` |
| `BASE_SUB` | string\|false | Subdirectory if app not in document root | `false` or `/kmp` |
| `SCRIPT_NAME` | string\|null | Script name for non-mod_rewrite servers | `/app.php` or null |

### Database Configuration

| Variable | Type | Purpose | Format |
|----------|------|---------|--------|
| `KMP_DB_DRIVER` | string | CakePHP database driver | `mysql`, `mariadb`, `postgres`, or `pgsql` (defaults to `mysql`) |
| `DATABASE_URL` | string | Primary database connection | `mysql://user:pass@host:port/db?encoding=utf8mb4` |
| `DATABASE_TEST_URL` | string | Test database connection | `mysql://user:pass@host:port/test_db` |
| `DB_HOST` | string | Database hostname | `localhost` |
| `DB_NAME` | string | Database name | `kmp_production` |
| `DB_USERNAME` | string | Database username | `kmp_user` |
| `DB_PASSWORD` | string | Database password | (sensitive) |
| `DB_PORT` | integer | Database port | `3306` |
| `DB_ENCODING` | string | Character encoding | `utf8mb4` |
| `DB_TIMEZONE` | string | Database timezone | `UTC` |

### Security Configuration

| Variable | Type | Purpose | Notes |
|----------|------|---------|-------|
| `SECURITY_SALT` | string | Cryptographic salt | 32+ random characters, keep secret |

**Generating a Security Salt:**

```bash
php -r "echo bin2hex(random_bytes(32)) . PHP_EOL;"
```

Output is a 64-character random string suitable for use as `SECURITY_SALT`.

### Email Configuration

| Variable | Type | Purpose | Example |
|----------|------|---------|---------|
| `EMAIL_TRANSPORT_DEFAULT_URL` | string | Complete email transport config | `smtp://user:pass@smtp.gmail.com:587?tls=true` |
| `EMAIL_FROM_ADDRESS` | string | Default sender email | `noreply@kingdom.example.com` |
| `EMAIL_FROM_NAME` | string | Default sender display name | `Kingdom Management Portal` |
| `SMTP_HOST` | string | SMTP server hostname | `smtp.gmail.com` |
| `SMTP_PORT` | integer | SMTP server port | `587` (TLS) or `465` (SSL) |
| `SMTP_USERNAME` | string | SMTP authentication username | (service account) |
| `SMTP_PASSWORD` | string | SMTP authentication password | (sensitive) |

**Email Transport Configuration Examples:**

```bash
# Gmail with App Password
EMAIL_TRANSPORT_DEFAULT_URL=smtp://user@gmail.com:app_password@smtp.gmail.com:587?tls=true

# Generic SMTP
EMAIL_TRANSPORT_DEFAULT_URL=smtp://username:password@mail.example.com:587?tls=true

# SMTP without authentication
EMAIL_TRANSPORT_DEFAULT_URL=smtp://mail.example.com:25
```

### Cache Configuration

| Variable | Type | Purpose | Example |
|----------|------|---------|---------|
| `CACHE_CAKECORE_URL` | string | CakePHP core cache service | `apcu://` or `redis://localhost:6379` |
| `CACHE_CAKEMODEL_URL` | string | Model metadata cache service | `apcu://` or `redis://localhost:6379` |

### Logging Configuration

| Variable | Type | Purpose | Format |
|----------|------|---------|--------|
| `LOG_DEBUG_URL` | string | External service for debug logs | `syslog://localhost:514` |
| `LOG_ERROR_URL` | string | External service for error logs | `syslog://localhost:514` |
| `LOG_QUERIES_URL` | string | External service for query logs | `syslog://localhost:514` |
| `LOG_PERFORMANCE_URL` | string | External service for request performance logs | `syslog://localhost:514` |
| `LOG_QUERIES_FILE_SIZE` | string | Maximum local query log size before rotation | `10MB` |
| `LOG_QUERIES_FILE_ROTATE` | integer | Number of rotated query log files to retain | `5` |
| `LOG_PERFORMANCE_FILE_SIZE` | string | Maximum local performance log size before rotation | `10MB` |
| `LOG_PERFORMANCE_FILE_ROTATE` | integer | Number of rotated performance log files to retain | `5` |
| `APPINSIGHTS_CONNECTION_STRING` | string | Azure Application Insights connection string for direct trace collection | `InstrumentationKey=...;IngestionEndpoint=https://...` |
| `APPINSIGHTS_LOG_ENABLED` | boolean | Send query and performance log channels directly to Application Insights | `true` / `false` |
| `APPINSIGHTS_CLOUD_ROLE` | string | Application Insights cloud role tag | `kmp` |
| `APPINSIGHTS_CLOUD_ROLE_INSTANCE` | string | Application Insights cloud role instance tag | host/container name |
| `APPINSIGHTS_LOG_TIMEOUT` | float | Timeout, in seconds, for direct Application Insights log export | `2.0` |
| `APPINSIGHTS_LOG_BATCH_SIZE` | integer | Number of trace events to batch before export | `25` |
| `APPINSIGHTS_ERROR_LOG_ENABLED` | boolean | Send web and CLI warning/error log events directly to Application Insights | `true` / `false` |
| `APPINSIGHTS_QUERY_LOG_ENABLED` | boolean | Send SQL query log events directly to Application Insights | `true` / `false` |
| `APPINSIGHTS_QUERY_SAMPLE_RATE` | float | Percentage of SQL query events to export directly | `10.0` |

### Performance Monitoring Configuration

| Variable | Type | Purpose | Example |
|----------|------|---------|---------|
| `PERF_REQUEST_LOG_ENABLED` | boolean | Enable request timing middleware logging | `true` / `false` |
| `PERF_SLOW_REQUEST_MS` | integer | Threshold for slow request warning logs | `750` |
| `PERF_LOG_ALL_REQUESTS` | boolean | Log all requests (not only slow ones) | `false` |
| `PERF_DB_QUERY_LOG_ENABLED` | boolean | Enable CakePHP SQL query timing logs on the primary DB connection | `true` / `false` |
| `PERF_KINGDOM_TAG` | string | Kingdom identifier attached to request timing metrics | `ansteorra` |

### Telemetry validation

After updating any `PERF_*`, `APPINSIGHTS_*`, or `LOG_*_FILE_*` variables,
run the deploy-time check to print the effective (non-secret) values,
verify that `LOGS` and `TMP` are writable under the current user, and
(optionally) emit a smoke trace:

```bash
cd app
bin/cake telemetry_check          # validate config + writability
bin/cake telemetry_check --send   # also write a smoke trace via Log::info
```

The smoke trace lands in `traces` (Application Insights) and
`logs/performance.log` (local).

### Environment variable precedence

KMP reads telemetry settings via CakePHP's `env()` helper. Precedence
when the same variable is set in multiple places:

1. Apache `SetEnv` / nginx `fastcgi_param` / FPM `env[NAME]` (process
   env). **Highest precedence.**
2. `app/config/.env` loaded by the application's bootstrap.
3. `app/config/app_local.php` defaults baked into config.

Use option 1 for kingdom-specific overrides on shared hosting where you
do not want to redeploy code to flip a flag. Use option 2 for
single-environment development. Re-run `bin/cake telemetry_check` to
confirm which value won after changes.

### Platform Audit WORM Mirror

| Variable | Type | Purpose | Safe default |
|----------|------|---------|--------------|
| `PLATFORM_AUDIT_WORM_SINK` | string | WORM mirror sink (`disabled`, `file`; Azure Blob documented for deployment wiring) | `disabled` |
| `PLATFORM_AUDIT_WORM_FILE_PATH` | string | Local/dev JSONL mirror path when `sink=file` | `app/tmp/platform_audit_worm.jsonl` |
| `PLATFORM_AUDIT_WORM_FAIL_CLOSED` | boolean | Fail the audit write if mirroring fails | `false` |
| `PLATFORM_AUDIT_WORM_AZURE_ACCOUNT` | string | Azure Storage account intended for immutable audit blobs | empty |
| `PLATFORM_AUDIT_WORM_AZURE_CONTAINER` | string | Azure Blob container with immutability policy/legal hold | empty |
| `PLATFORM_AUDIT_WORM_AZURE_PREFIX` | string | Blob key prefix for audit records | `platform-audit/` |
| `PLATFORM_AUDIT_WORM_AZURE_IMMUTABILITY_POLICY` | string | Name/description of the deployed retention/legal-hold policy | empty |

The WORM mirror is disabled by default so production does not silently write
audit copies to ephemeral disk. Local development can set `sink=file`; records
are append-only JSONL with per-event digests and a mirror hash chain. Production
Azure deployments should provision a dedicated private Blob container with
versioning disabled as appropriate, time-based immutable retention or legal
hold, no delete permissions for the application identity, and lifecycle/monitoring
alerts before enabling fail-closed mirroring.

### Platform Admin Portal

The `/platform-admin` prefix is disabled by default and is intended only for a
separate Container App or reserved platform-admin host. The tenant-facing app
should leave it disabled unless that instance is also serving the configured
admin hostname.

| Variable | Type | Purpose | Safe default |
|----------|------|---------|--------------|
| `KMP_PLATFORM_ADMIN_PORTAL_ENABLED` | boolean | Enables the read-only platform admin surface | `false` |
| `KMP_PLATFORM_ADMIN_HOSTS` | CSV | Hostnames reserved for platform admin | `platform.kmp.localhost` |
| `KMP_PLATFORM_ADMIN_ALLOWED_STATUSES` | CSV | Allowed `platform_users.status` values | `active` |
| `KMP_PLATFORM_ADMIN_DETAILED_LOGIN_ERRORS` | boolean | Shows password-vs-MFA login diagnostics for local/staging troubleshooting | `true` for local/staging, otherwise `false` |

Never pass tenant-session cookies or public traffic to the admin Container App.
Requests to a configured
`KMP_PLATFORM_ADMIN_HOSTS` hostname can use `/platform-admin/login` with a
platform user password and TOTP code. The app denies access unless the portal
flag is enabled and the matching platform user has an allowed status. Dashboard
views intentionally omit database hosts, roles, object URIs, wrapped encryption
keys, secret indexes, and raw job errors.

Leave `KMP_PLATFORM_ADMIN_DETAILED_LOGIN_ERRORS=false` in production. Enable it
only in local or staging troubleshooting windows when operators need to
distinguish password verification failures from MFA code failures.

For local development, bootstrap the first admin after platform migrations:

```bash
bin/cake platform admin bootstrap --email admin@example.org
```

The bootstrap command prints the initial password, TOTP bootstrap secret, and
recovery codes once. Capture them in a trusted password manager before closing
the terminal.

If the password output was not captured, generate a replacement password:

```bash
bin/cake platform admin reset-password \
  --email admin@example.org \
  --reason "bootstrap output was not captured"
```

Then set `KMP_PLATFORM_ADMIN_PORTAL_ENABLED=true` and
`KMP_PLATFORM_ADMIN_HOSTS=platform.kmp.localhost`. Visit
`http://platform.kmp.localhost:8080/platform-admin`; non-admin paths on that
host redirect to the platform admin portal.

### Document Storage Configuration

| Variable | Type | Purpose | Example |
|----------|------|---------|---------|
| `AZURE_STORAGE_AUTH_MODE` | string | Azure Blob auth mode (`managedIdentity` or `connectionString`) | `managedIdentity` |
| `AZURE_STORAGE_ACCOUNT_NAME` | string | Storage account name for managed identity auth | `kmpdocs...` |
| `AZURE_STORAGE_CONTAINER_PREFIX` | string | Per-tenant blob container prefix | `documents` |
| `AZURE_STORAGE_CONNECTION_STRING` | string | Legacy/dev Azure Blob credentials | (avoid in Azure) |
| `AWS_S3_BUCKET` | string | S3 bucket name | `kmp-documents-prod` |
| `AWS_DEFAULT_REGION` | string | S3 region | `us-east-1` |
| `AWS_ACCESS_KEY_ID` | string | S3 access key (optional with IAM role) | `AKIA...` |
| `AWS_SECRET_ACCESS_KEY` | string | S3 secret key (optional with IAM role) | (sensitive) |
| `AWS_SESSION_TOKEN` | string | Temporary credentials token (optional) | (sensitive) |
| `AWS_S3_PREFIX` | string | Optional key prefix for document paths | `kmp/documents/` |
| `AWS_S3_ENDPOINT` | string | Custom S3 endpoint (optional, MinIO etc.) | `https://minio.example.com` |
| `AWS_S3_USE_PATH_STYLE_ENDPOINT` | boolean | Force path-style S3 URLs | `false` |

The storage adapter (`local`, `azure`, or `s3`) is configured in `config/app_local.php` under
`Documents.storage.adapter`, not via environment variable. Azure Container Apps should use
managed identity and Storage Blob Data Contributor RBAC rather than storage account keys.

## Environment Variable Files

### .env File Format

Environment variables are loaded from `.env` files in the `config/` directory:

```bash
# .env - Development environment (NOT version controlled)

# Debug mode
DEBUG=true

# Application settings
APP_DEFAULT_TIMEZONE=UTC
APP_ENCODING=UTF-8

# Database
KMP_DB_DRIVER=mysql
DATABASE_URL=mysql://root:root@localhost/kmp_dev?encoding=utf8mb4

# Security
SECURITY_SALT=ARandomlyGeneratedSalt1234567890123456789012345678

# Email (development - uses Debug transport)
EMAIL_FROM_ADDRESS=dev@localhost
EMAIL_FROM_NAME=KMP Development

# Logging (development)
LOG_DEBUG_URL=
LOG_ERROR_URL=
LOG_QUERIES_URL=
LOG_PERFORMANCE_URL=
LOG_QUERIES_FILE_SIZE=10MB
LOG_QUERIES_FILE_ROTATE=5
LOG_PERFORMANCE_FILE_SIZE=10MB
LOG_PERFORMANCE_FILE_ROTATE=5
APPINSIGHTS_CONNECTION_STRING=
APPINSIGHTS_LOG_ENABLED=false
APPINSIGHTS_CLOUD_ROLE=kmp
APPINSIGHTS_CLOUD_ROLE_INSTANCE=
APPINSIGHTS_LOG_TIMEOUT=2.0
APPINSIGHTS_LOG_BATCH_SIZE=25
APPINSIGHTS_ERROR_LOG_ENABLED=true
APPINSIGHTS_QUERY_LOG_ENABLED=false
APPINSIGHTS_QUERY_SAMPLE_RATE=10.0

# Performance monitoring (off by default in dev)
PERF_REQUEST_LOG_ENABLED=false
PERF_SLOW_REQUEST_MS=750
PERF_LOG_ALL_REQUESTS=false
PERF_DB_QUERY_LOG_ENABLED=false
PERF_KINGDOM_TAG=ansteorra
```

### Configuration Hierarchy

KMP loads configuration in this order (see `config/bootstrap.php`):

1. `.env` file - Loaded via `josegonzalez/dotenv`, sets environment variables
2. `config/app.php` - Baseline configuration (uses `env()` calls to read from step 1)
3. `config/app_local.php` - Environment-specific overrides (if present)
4. `config/app_queue.php` - Queue plugin configuration (if present)

Later values override earlier ones. Environment variables set in `.env` are consumed by `env()` calls in `app.php`.

## Environment-Specific Examples

### Development Environment

```bash
# .env

DEBUG=true
DATABASE_URL=mysql://root:root@localhost/kmp_dev?encoding=utf8mb4&timezone=UTC
SECURITY_SALT=dev_salt_not_secure_change_in_production
APP_DEFAULT_TIMEZONE=UTC

# Email - use Debug transport (files capture emails without sending)
EMAIL_FROM_ADDRESS=dev@localhost
EMAIL_FROM_NAME=KMP Development

# Logging
LOG_DEBUG_URL=
LOG_ERROR_URL=
LOG_PERFORMANCE_URL=
```

### Staging Environment

```bash
# .env (Staging)

DEBUG=false
DATABASE_URL=mysql://staging_user:secure_password@staging-db.internal/kmp_staging?encoding=utf8mb4
SECURITY_SALT=staging_salt_must_be_cryptographically_random_64chars
APP_DEFAULT_TIMEZONE=America/Chicago

# Email - use staging SMTP server
EMAIL_TRANSPORT_DEFAULT_URL=smtp://staging@example.com:password@mail.staging.example.com:587?tls=true
EMAIL_FROM_ADDRESS=noreply-staging@kingdom.example.com
EMAIL_FROM_NAME=KMP Staging

# Logging to remote service
LOG_ERROR_URL=syslog://log-aggregator.internal:514
LOG_PERFORMANCE_URL=syslog://log-aggregator.internal:514
```

### Production Environment

```bash
# .env (Production)

DEBUG=false
DATABASE_URL=mysql://prod_user:very_secure_password@prod-db.internal/kmp_production?encoding=utf8mb4&timezone=UTC
SECURITY_SALT=very_long_cryptographically_random_production_salt_here
APP_DEFAULT_TIMEZONE=America/Chicago

# Email - use production SMTP service
EMAIL_TRANSPORT_DEFAULT_URL=smtp://noreply@kingdom.com:secure_password@smtp.office365.com:587?tls=true
EMAIL_FROM_ADDRESS=noreply@kingdom.org
EMAIL_FROM_NAME=Kingdom Management Portal

# Logging to cloud service
LOG_ERROR_URL=syslog://logs.cloud-provider.com:514?protocol=tls
LOG_DEBUG_URL=syslog://logs.cloud-provider.com:514?protocol=tls
LOG_PERFORMANCE_URL=syslog://logs.cloud-provider.com:514?protocol=tls
LOG_QUERIES_FILE_SIZE=10MB
LOG_QUERIES_FILE_ROTATE=5
LOG_PERFORMANCE_FILE_SIZE=10MB
LOG_PERFORMANCE_FILE_ROTATE=5
APPINSIGHTS_CONNECTION_STRING=InstrumentationKey=...;IngestionEndpoint=https://...
APPINSIGHTS_LOG_ENABLED=true
APPINSIGHTS_CLOUD_ROLE=kmp
APPINSIGHTS_CLOUD_ROLE_INSTANCE=
APPINSIGHTS_LOG_TIMEOUT=2.0
APPINSIGHTS_LOG_BATCH_SIZE=25
APPINSIGHTS_ERROR_LOG_ENABLED=true
APPINSIGHTS_QUERY_LOG_ENABLED=false
APPINSIGHTS_QUERY_SAMPLE_RATE=10.0

# Caching
CACHE_CAKECORE_URL=redis://prod-redis-1.internal:6379
CACHE_CAKEMODEL_URL=redis://prod-redis-1.internal:6379
```

## Security Best Practices

### Securing Environment Variables

1. **Never commit `.env` files** - Add `*.env` to `.gitignore`
2. **Restrict file permissions** - `.env` should only be readable by app process:
   ```bash
   chmod 600 .env
   ```

3. **Use strong values** - Generate security salts and API keys with cryptographic randomness
4. **Rotate regularly** - Change `SECURITY_SALT` and password values periodically
5. **Use managed secrets** - Platforms like AWS Secrets Manager, Azure Key Vault, or HashiCorp Vault
6. **Audit access** - Monitor which systems/users access environment variables
7. **Different per environment** - Each environment (dev, staging, prod) has unique credentials

### Sensitive Variables

These variables contain sensitive data and require special protection:

- `SECURITY_SALT` - Cryptographic key material
- `DATABASE_URL` - Database credentials
- `DB_PASSWORD` - Direct database password
- `SMTP_PASSWORD` - Email service credentials
- `EMAIL_TRANSPORT_DEFAULT_URL` - May contain email credentials
- `AZURE_STORAGE_CONNECTION_STRING` - Storage account credentials
- `AWS_SECRET_ACCESS_KEY` - S3 secret credentials
- `AWS_SESSION_TOKEN` - Temporary S3 credentials token

### Development Safety

In development environments:

- Use default credentials (user: `root`, password: `root`)
- Enable `DEBUG=true` for full error reporting
- Accept self-signed SSL certificates
- Store in project `.env` (added to `.gitignore`)

### Production Safety

In production environments:

- **Disable debug**: Set `DEBUG=false`
- **Use strong credentials**: Random 32+ character passwords
- **Use environment-specific values**: Never reuse development credentials
- **Store securely**: Use cloud provider secret management
- **Restrict access**: Only the application process can read the `.env` file
- **Use TLS**: Require TLS for database and SMTP connections
- **Monitor changes**: Alert on environment variable modifications

## Variable Types and Casting

The `env()` function loads variables as strings. Use `filter_var()` for proper type conversion:

```php
// Boolean conversion
$debug = filter_var(env("DEBUG", false), FILTER_VALIDATE_BOOLEAN);

// Integer conversion
$port = (int)env("SMTP_PORT", 25);

// URL parsing
$dbUrl = env("DATABASE_URL");
```

CakePHP automatically handles casting for built-in configurations like:
- `DATABASE_URL` (parsed to connection array)
- `EMAIL_TRANSPORT_DEFAULT_URL` (parsed to transport settings)

## Environment Variable Loading Behavior

### .env File Loading

1. Look for `.env` in `config/` directory
2. Load variables into $_ENV and available to `env()`
3. Variables already in environment take precedence
4. Not found in `.env` defaults to `getenv()`

### Recommended .env Approach

Place in `config/.env`:

```bash
# config/.env

# Copy from .env.example and customize for your environment
DEBUG=false
DATABASE_URL=mysql://...
SECURITY_SALT=...
```

Add to `.gitignore`:

```
config/.env
config/.env.local
.env
.env.local
```

## Troubleshooting

### Environment Variable Not Loading

1. **Check file location**: Should be in `config/` directory
2. **Verify file syntax**: No spaces around `=` in assignments
3. **Check file permissions**: File must be readable by web process
4. **Clear cache**: Run `bin/cake cache clear_all`
5. **Restart web server**: PHP-FPM or Apache needs restart for changes

### Incorrect Values Being Used

1. **Check variable name**: Ensure correct spelling and case
2. **Check .env precedence**: Variables in shell environment take precedence
3. **Clear APCu cache**: `bin/cake cache clear_all` (APCu caches variable access)
4. **Verify .gitignore**: Ensure no committed `.env` files override local `.env`

### Database Connection Issues

1. **Verify driver and DATABASE_URL format**: `KMP_DB_DRIVER=mysql` with `mysql://user:pass@host:port/db`, or `KMP_DB_DRIVER=postgres` with `postgres://user:pass@host:port/db`
2. **Test credentials manually**: `mysql -u user -p -h host db_name`
3. **Check firewall**: Ensure host/port is accessible from app server
4. **Verify encoding**: Include `?encoding=utf8mb4` in connection string

## See Also

- [Configuration Guide](2-configuration.md)
- [Deployment Guide](8-deployment.md)
- [Security Best Practices](7.1-security-best-practices.md)
- [CakePHP Configuration Documentation](https://book.cakephp.org/5/en/development/configuration.html)
