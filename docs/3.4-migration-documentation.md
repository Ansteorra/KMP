...
The KMP database schema is managed with CakePHP Migrations 5's built-in database backend. This documentation covers the migration history, patterns, and evolution of the database schema from initial setup to current state.

## Migration Framework

### Technology Stack

- **Migration Engine**: CakePHP Migrations 5 built-in backend
...
- **Seed Execution**: `bin/cake seeds run SeedName`
- **History Storage**: Legacy core/plugin `phinxlog` tables are intentionally retained through
  `Migrations.legacyTables` for tenant schema-version and backup/restore compatibility
...
Once a migration has been applied in any shared environment, treat that file as immutable. Do not edit its schema operations, seed data, or `down()` behavior to correct a mistake; add a new forward-only corrective migration instead. A dependency upgrade may mechanically replace an unavailable migration base class only when the migration operations remain unchanged and the full migration catalog is verified. During an unreleased feature branch cleanup, intermediate migrations may be squashed only after confirming no shared environment has applied them and after preserving any legacy backup/restore hydration paths that production data migration depends on.
