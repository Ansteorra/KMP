...
Once a migration has been applied in any shared environment, treat that file as immutable. Do not edit its schema operations, seed data, or `down()` behavior to correct a mistake; add a new forward-only corrective migration instead. During an unreleased feature branch cleanup, intermediate migrations may be squashed only after confirming no shared environment has applied them and after preserving any legacy backup/restore hydration paths that production data migration depends on.
