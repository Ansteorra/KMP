...
- [Crawler and Probe Controls](#crawler-and-probe-controls)
...
## Crawler and Probe Controls

KMP is an internal portal with two intentionally indexable public surfaces:

- `/events`
- `/gatherings/public-landing/*`

`app/webroot/robots.txt` disallows all other paths for cooperative crawlers. Apache also applies `X-Robots-Tag: noindex, nofollow, noarchive` to every response outside those public surfaces.

The Apache rewrite rules provide low-cost origin protection before CakePHP starts:

- Identified crawler and scanner user agents receive HTTP 403 outside the public allowlist.
- Common secret-file, executable PHP, WordPress, phpMyAdmin, PHPUnit, CGI, and dependency-manifest probes receive HTTP 404.
- Existing static files remain available so public pages can load their assets.

Public machine-consumed endpoints such as calendar feeds, calendar downloads, health checks, OpenAPI output, Branches API responses, and the Officers CSV remain accessible to their clients but are not indexable. These Apache controls reduce PHP and database work from routine crawlers; they are not a substitute for an edge WAF when traffic does not identify itself or targets arbitrary valid application routes.
