...
**Virtual fields:** `is_expired`, `is_active`, `is_declined`, `can_be_declined` (within 90 days, not already declined, not expired/deleted), `status_badge_class`, `status_display`
...
|------------|------|---------|
| `waiver-upload-wizard` | `waiver-upload-wizard-controller.js` | Multi-step desktop/mobile upload wizard. Uses PDF.js for preview, attempts to resize and convert supported images to grayscale JPEGs in the browser, reports preparation and measured network upload progress, then identifies server conversion as a separate phase. |
...
Browser preprocessing is an optimization, not a trust boundary. PDFs pass through unchanged, images that cannot be processed in the browser fall back to their original files with visible status feedback, and `ImageToPdfConversionService` still performs the final server-side resize, grayscale/contrast conversion, and PDF generation. Converted images are embedded at 150 DPI using JPEG quality 80 while retaining letter-size PDF page dimensions.
