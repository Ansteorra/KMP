...
This document provides documentation for the Officers plugin service layer, including the `OfficerManagerInterface` and its default implementation `DefaultOfficerManager`.

## Service Architecture Overview

The Officers plugin uses a service-oriented architecture with dependency injection for workflow-shared officer lifecycle operations. The primary service interface is `OfficerManagerInterface`, which is implemented by `DefaultOfficerManager`.
...
| `TriggerDispatcher` | Emits release workflow triggers from shared service logic |
| `QueuedMailerAwareTrait` | Asynchronous notification processing |

## OfficerManagerInterface

The `OfficerManagerInterface` defines the contract for non-hire officer lifecycle operations that are still shared outside the workflow engine.
...
The `DefaultOfficerManager` is the standard implementation providing shared release and recalculation behavior.
...
- `$triggerDispatcher` - Workflow trigger dispatcher used for release notifications/events

---

## Assignment Operations

Officer assignment is now workflow-only. Runtime hiring enters through the `officer-hire` workflow definition via `OfficersController::assign()`, not through `OfficerManagerInterface`.

That workflow is responsible for:

1. warrant validation before any conflict resolution,
2. unique-office overlap handling,
3. officer record creation,
4. warrant roster creation when required, and
5. hire/replacement notifications.

The service layer documented here remains responsible for shared release/recalculation behavior that is reused by workflow actions and office maintenance flows.
