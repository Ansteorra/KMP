...
5. **Use Table Metadata Mode**: Let inner `*-table` Turbo frame refreshes skip view/filter metadata reloads
6. **Cache Filter Options**: Reuse dropdown `filterOptionsSource` lookups per request
7. **Prefer Lean Table State**: Avoid pretty-printed state payloads and send only table-changing state on refresh
8. **Treat COUNT as Expensive**: For join-heavy grids, consider simplified pagination or deferred totals
