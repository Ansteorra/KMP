...
Decrypts one managed Platform Admin database archive with its separately
escrowed, backup-scoped recovery-key file. It verifies the archive checksum and
authenticated identity, refuses to overwrite an existing output, and creates an
owner-only PostgreSQL custom-format dump.
...
**When to use:** Only on a secured recovery host during external platform
database disaster recovery. See [Backup & Restore](deployment/backup-restore.md#portable-per-backup-recovery-keys).
...
|---------|---------------------|---------|
| `platform worker run` | Every minute in ACA; continuous bounded cycles locally | Dispatch due schedules and drain default, active-tenant, and platform queues from one authority |
| `platform schedule due` | Manual diagnostics | Dispatch only due multi-tenant timed workflow and maintenance schedules |
| `platform queues run` | Manual diagnostics | Drain only default, active-tenant, and platform queues |
| `queue run -q` | Every 2 minutes | Process one database queue in single-tenant mode |
| `workflow_scheduler` | Every minute | Dispatch due scheduled workflow definitions |
...
| `tenant_backups_enqueue` | Daily (tenant-backup-fleet schedule) | Enqueue managed backups for active tenants per the global backup policy |

The local Docker stack installs these cron entries automatically unless `KMP_SKIP_CRON=true` is set. See `cronScripts/` in the project root for shared-host cron wrapper examples.
