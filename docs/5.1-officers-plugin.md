...
The `officer-hire` workflow now implements the core assignment business logic:

```php
// Assignment enters through the workflow trigger dispatched by OfficersController::assign()
$results = $triggerDispatcher->dispatch('Officers.HireRequested', [
    'memberId' => $memberId,
    'officeId' => $officeId,
    'branchId' => $branchId,
    'startOn' => $startOn,
...
- Member profile office lists show the current warrant expiration (or the latest expired warrant date) and expose an expandable history for each assignment
