...
```
GET /gatherings/feed[?branch={public_id}&type={gathering_type_id}]
```
...
|-----------|-------------|---------|
| `branch` | Filter by branch public ID | `?branch=mfoBwnBC` |
| `type` | Filter by gathering type ID | `?type=3` |

Parameters can be combined: `?branch=mfoBwnBC&type=3`

### Feed Behavior

- **Content:** Non-cancelled, non-deleted gatherings from 30 days ago onward
...
1. Builds the feed URL based on current filter selections (branch, type)
