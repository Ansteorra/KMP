...
**Related Docs:** `docs/gathering-location-maps.md`, `docs/osm-csp-configuration.md`
...
- **Calendar (`/gatherings/calendar`):** Month/week/list views, filter sidebar, and quick-view modal. Stimulus handles AJAX requests for attendance and detail snippets without page reloads. Quick view includes "Add to Calendar" button.
- **View (`/gatherings/view/{id}`):** Comprehensive tabbed layout with the following tabs:
  ...
  - **Schedule:** Event timeline and schedule information (order: 4)
...
- **Public Landing (`/gatherings/public-landing/{publicId}`):** Public-facing event page with:
  - Hero section with event name, type, and dates
...
- **Location Maps:** The map tab (`element/gatherings/mapTab.php`) uses stored latitude/longitude to render an interactive map powered by Leaflet and OpenStreetMap. CSP headers in `Application.php` allow tile loading. See `docs/gathering-location-maps.md` and `docs/osm-csp-configuration.md` for details.
...
- **Public ID System:** Gatherings use the application-wide Public ID system (see `docs/PUBLIC_ID_SYSTEM.md`) for generating UUIDs used in public URLs. Public IDs are automatically generated on first save and remain stable even if event details change.
...
3. **Calendar Subscriptions:** Generate calendar feeds for automatic updates (beyond one-time .ics downloads).
4. **Reminder Emails:** Notify attendees as gatherings approach using stored attendance preferences.
5. **Staff Scheduling:** Expand staff management to include shift scheduling and volunteer coordination.
6. **Event Time Customization:** Allow organizers to specify exact event times instead of default 9 AM - 5 PM.
7. **Advanced Map Features:** Add route planning, nearby amenities, parking information.
8. **Public Registration:** Allow non-authenticated users to pre-register via public landing page.
...
- [Gathering Location Maps](gathering-location-maps.md) - Detailed map integration documentation
- [Gathering Location Maps Quick Reference](gathering-location-maps-quick-reference.md)
- [Gatherings Calendar](gatherings-calendar.md) - Calendar views and features
- [Gatherings Calendar Quick Reference](gatherings-calendar-quick-reference.md)
- [Template Gathering Activities](template-gathering-activities-summary.md) - Activities plugin integration
- [Template Gathering Activities Quick Reference](template-gathering-activities-quick-reference.md)
- [Gathering Schedule Implementation](gathering-schedule-implementation-summary.md)
- [OSM CSP Configuration](osm-csp-configuration.md) - Map security configuration
- [PUBLIC_ID_SYSTEM](PUBLIC_ID_SYSTEM.md) - Public ID architecture (if exists)
