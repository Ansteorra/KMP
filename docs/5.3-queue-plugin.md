...
# Bounded one-shot worker for automation
bin/cake queue run --max-jobs 25 --max-runtime 45 --exit-when-empty
```

`--max-jobs` limits attempted jobs, `--max-runtime` is measured in seconds,
and `--exit-when-empty` exits without sleeping when no due job remains. In a
multi-tenant installation, do not use this command as the fleet worker: queue
tables belong to the default and tenant databases. Run
`bin/cake platform worker run` instead so one background authority dispatches
timed work, binds and drains the default plus active-tenant datasources, and
claims queued platform jobs.
...
'Queue' => [
    'workermaxruntime' => 110,      // Max runtime before restart (seconds)
