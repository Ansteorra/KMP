...
3. PHP binary with required extensions (default path `/usr/local/php84/bin/php`).
...
| `--php-bin PATH` | PHP executable to use (default `/usr/local/php84/bin/php` or `CAKE_PHP_BIN`). |
...
CAKE_PHP_BIN=/usr/local/php84/bin/php \
...
💡 **Tip:** Add `--php-bin /usr/local/php84/bin/php` if the host default differs from our shared server paths.
