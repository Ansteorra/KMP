...
class MemberAuthorizationDetailsJSONCell extends Cell
{
    ...
    // phpcs:disable SlevomatCodingStandard.TypeHints.ReturnTypeHint.MissingNativeTypeHint -- Preserve CakePHP action signature.
    /**
     ...
     * Preserve the inherited CakePHP cell action signature.
     */
    public function display($id)
    {
        ...
        foreach ($currentAuths as $auth) {
            ...
            $offlineAuthorizations[] = [
                'label' => $activityGroup . ': ' . $activityName,
                'expires_on' => $auth->expires_on?->format('Y-m-d'),
            ];
        }
        ...
        if (!empty($permissionIds)) {
            ...
            foreach ($activities as $activity) {
                ...
                $offlineApprover[] = ['label' => $activityGroup . ': ' . $activityName, 'expires_on' => null];
            }
        } else {
            $organizedAuthorisor = [];
        }
        $responseData = ['Can Authorize' => $organizedAuthorisor, 'Authorizations' => $organizedAuths,];
        $responseData['offline_sections'] = [
            ['title' => 'Can Authorize (last verified)', 'items' => $offlineApprover],
            ['title' => 'Authorizations', 'items' => $offlineAuthorizations],
        ];
