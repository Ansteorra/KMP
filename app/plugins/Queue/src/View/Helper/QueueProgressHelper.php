...
class QueueProgressHelper extends Helper
{
    ...
    public function htmlProgressBar(QueuedJob $queuedJob, ?string $fallbackHtml = null): ?string
    {
        ...
        return '<progress value="' . number_format($progress * 100, 0) . '" max="100" title="' . $title . '">'
            . $fallbackHtml . '</progress>';
    }
    ...
    protected function readStatistics(): array
    {
        ...
        if (!Configure::read('debug')) {
            $queuedJobStatistics = Cache::read(TenantAwareCache::tenantScopedKey(static::KEY), static::CONFIG);
        }
        if ($queuedJobStatistics === false) {
            ...
            Cache::write(TenantAwareCache::tenantScopedKey(static::KEY), $queuedJobStatistics, static::CONFIG);
        }
