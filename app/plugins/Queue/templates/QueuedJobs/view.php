...
echo $this->KMP->getAppSetting("KMP.ShortSiteTitle") . ': Queueing Engine';
$this->KMP->endBlock(); ?>
...
<nav class="actions" id="actions-sidebar">
    <ul class="nav nav-pills">
        ...
        <li class="nav-item px-1">
            <?= $this->Html->link(__d('queue', 'Export'), ['action' => 'view', $queuedJob->id, '_ext' => 'json', '?' => ['download' => true]], ["class" => "btn btn-primary btn-sm"]) ?>
        </li>

        <?php if (!$queuedJob->completed) { ?>
            <li class="nav-item px-1">
                <?= $this->Html->link(__d('queue', 'Edit Queued Job'), ['action' => 'edit', $queuedJob->id], ["class" => "btn btn-primary btn-sm"]) ?>
            </li>
        <?php } ?>
        <li class="nav-item px-1">
            <?= $this->Form->postLink(__d('queue', 'Delete Queued Job'), ['action' => 'delete', $queuedJob->id], ['confirm' => __d('queue', 'Are you sure you want to delete # {0}?', $queuedJob->id), "class" => "btn btn-danger btn-sm"]) ?>
        </li>
        <li class="nav-item px-1">
            <?= $this->Html->link(__d('queue', 'List {0}', __d('queue', 'Queued Jobs')), ['action' => 'index'], ["class" => "btn btn-primary btn-sm"]) ?>
        </li>
    </ul>
</nav>
