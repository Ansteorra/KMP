...
<tr>
    ...
    <td><?= $award->hasValue('domain')
        ? $this->Html->link($award->domain->name, [
            'controller' => 'Domains',
            'action' => 'view',
            $award->domain->id,
        ])
...
<tr>
    ...
    <td><?= $award->hasValue('level')
        ? $this->Html->link($award->level->name, [
            'controller' => 'Levels',
            'action' => 'view',
            $award->level->id,
        ])
...
<tr>
    ...
    <td><?= $award->hasValue('branch')
        ? $this->Html->link($award->branch->name, [
            'plugin' => null,
            'controller' => 'Branches',
            'action' => 'view',
            $award->branch->public_id,
        ])
        : '' ?>
    </td>
</tr>
<tr>
    <th scope="row"><?= __('Approval Process') ?></th>
    <td>
        <?php if ($award->hasValue('approval_process')) : ?>
            <?= $this->Html->link(
                h($award->approval_process->name),
                ['controller' => 'ApprovalProcesses', 'action' => 'view', $award->approval_process->id],
            ) ?>
        <?php else : ?>
            <span class="text-muted"><?= __('Legacy recommendation state process') ?></span>
        <?php endif; ?>
    </td>
</tr>
<tr>
...
<div class="row g-3">
    <div class="col-12">
        <fieldset class="border rounded-3 bg-white shadow-sm p-3 h-100">
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6"><?php echo $this->Form->control('abbreviation'); ?></div>
                <div class="col-12"><?php echo $this->Form->control('description'); ?></div>
    ...
    <div class="col-12 col-xl-6">
        <fieldset class="border rounded-3 bg-white shadow-sm p-3 h-100">
            <legend class="float-none w-auto px-2 fs-6 fw-semibold mb-3">
                <i class="bi bi-palette text-success me-1" aria-hidden="true"></i>
                <?= __('Insignia & Specialties') ?>
            </legend>
            <?php
            ...
            <div class="mb-3 form-group specialties">
                ...
                <div class="input-group">
                    <input type="text" id="specialtyInput" data-awards-award-form-target="new" class="form-control" placeholder="Add Specialty">
