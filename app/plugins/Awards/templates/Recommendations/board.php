                    ...
                    <td class="sortable" width="14.28%" data-status="<?= h($statusName) ?>">
                        ...
                        if (is_array($status)) :
                            foreach ($status as $recommendation) : ?>
                                <div class="card m-1" style="cursor: pointer;" draggable="true" data-stackRank="<?= $recommendation->stack_rank ?>" data-recId="<?= $recommendation->id ?>" id="card_<?= $recommendation->id ?>">
                                    <div class="card-body">
                                        <div class="card-title">
                                            <?= $this->Html->link($recommendation->award->name, ['action' => 'view', $recommendation->id]) ?>
                                            <button type="button" class="btn btn-primary btn-sm float-end" data-bs-toggle="modal" data-bs-target="#editModal" onclick="loadRec(<?= $recommendation->id ?>, '<?= $this->request->getRequestTarget(); ?>')">Edit</button>
                                        </div>
