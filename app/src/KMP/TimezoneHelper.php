 ...
 *
 * Provides centralized timezone handling for the KMP application. This helper manages
 * timezone conversions for both display (UTC -> User TZ) and input (User TZ -> UTC)
 * following a priority-based timezone resolution system.
 *
 * ## Timezone Priority
 *
 * The helper resolves timezones in the following order:
 * 1. **User Timezone**: Individual member's preferred timezone (from members.timezone)
 * 2. **Application Default**: System-wide default timezone (from AppSettings: KMP.DefaultTimezone)
 * 3. **UTC Fallback**: Universal Coordinated Time (always available)
 *
 * ## Core Functionality
 *
 * ### Display Conversion (Way Out)
 * Converts UTC-stored dates/times to user's local timezone for display:
 * ```php
 * $utcDateTime = DateTime::parse('2025-01-15 14:30:00'); // Stored in DB as UTC
 * $displayTime = TimezoneHelper::toUserTimezone($utcDateTime, $currentUser);
 * echo $displayTime->format('Y-m-d H:i:s'); // "2025-01-15 08:30:00" (Central)
 * ```
 *
 * ### Input Conversion (Way In)
 * Converts user's local timezone input to UTC for storage:
 * ```php
 * $userInput = '2025-01-15 08:30:00'; // User enters in their local time
 * $utcDateTime = TimezoneHelper::toUtc($userInput, 'America/Chicago');
 * // Stored as "2025-01-15 14:30:00" UTC
 * ```
 *
 * ## Usage Examples
 *
 ...
 *
 * // Get user's timezone
 * $userTz = TimezoneHelper::getUserTimezone($member);
 * // Returns: "America/Chicago" or "America/New_York" or "UTC"
 *
 * // Convert for display
 * $localTime = TimezoneHelper::toUserTimezone($gathering->start_date, $member);
 * echo TimezoneHelper::formatDateTime($localTime);
 * ```
 *
 * ### Form Input Handling
 * ```php
 * // When receiving form data in user's timezone
 * $gatheringData = [
 *     'start_date' => '2025-03-15 09:00:00', // User's local time
 *     'end_date' => '2025-03-17 17:00:00'
 * ];
 *
 * // Convert to UTC before saving
 * $gatheringData['start_date'] = TimezoneHelper::toUtc(
 *     $gatheringData['start_date'],
 *     TimezoneHelper::getUserTimezone($currentUser)
 * );
 * $gatheringData['end_date'] = TimezoneHelper::toUtc(
 *     $gatheringData['end_date'],
 *     TimezoneHelper::getUserTimezone($currentUser)
 * );
 * ```
 *
 * ### Template Display
 * ```php
 * // In a template (use TimezoneViewHelper instead for templates)
 * $displayDate = TimezoneHelper::formatForDisplay(
 *     $gathering->start_date,
 *     $currentUser,
 *     'l, F j, Y g:i A T' // Include timezone abbreviation
 * );
 * // "Saturday, March 15, 2025 9:00 AM CDT"
 * ```
 *
 * ## Method Reference
 *
 * ### Timezone Resolution
 * - `getUserTimezone(member, default)` - Get timezone for a specific user/member
 * - `getAppTimezone()` - Get application default timezone from settings
 * - `getTimezoneWithFallback(timezone)` - Validate and provide fallback
 *
 * ### Conversion Methods
 * - `toUserTimezone(datetime, member)` - Convert UTC to user's timezone
 * - `toUtc(datetime, timezone)` - Convert from timezone to UTC
 * - `convertBetweenTimezones(datetime, from, to)` - Direct timezone conversion
 *
 * ### Formatting Methods
 * - `formatForDisplay(datetime, member, format)` - Format for user display
 * - `formatDateTime(datetime, format)` - Standard datetime formatting
 * - `formatDate(datetime, format)` - Date-only formatting
 * - `formatTime(datetime, format)` - Time-only formatting
 *
 * ### Utility Methods
 * - `isValidTimezone(timezone)` - Validate timezone identifier
 * - `getTimezoneList()` - Get list of common timezones
 * - `getTimezoneAbbreviation(datetime, timezone)` - Get TZ abbreviation (CDT, EST, etc.)
 *
 * ## Integration Points
 *
 * ### View Helper
 * The TimezoneViewHelper provides template-friendly wrappers:
 * ```php
 * // In templates
 * <?= $this->Timezone->format($gathering->start_date, 'F j, Y g:i A T') ?>
 * ```
 *
 * ### JavaScript Integration
 * Client-side timezone handling via timezone-utils.js:
 * ```javascript
 ...
 *
 * ### Form Helpers
 * Automatic timezone conversion for datetime inputs:
 * ```php
 ...
 * - Always validate timezone identifiers against DateTimeZone list
 * - Sanitize timezone input from user forms
 * - Store in UTC to prevent timezone manipulation attacks
 * - Log timezone conversions for audit trails when needed
 *
 * ## Performance Notes
 * - Timezone list is cached for performance
 * - Consider caching user timezone in session
 * - Batch conversions when processing multiple dates
 *
 * @package App\KMP
 * @see \App\View\Helper\TimezoneHelper View helper for templates
 * @see \App\Model\Entity\Member Member entity with timezone field
 */
class TimezoneHelper
{
     ...
     *
     * @param \Cake\I18n\DateTime|string|null $datetime Datetime in source timezone
