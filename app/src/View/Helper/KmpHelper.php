...
class KmpHelper extends Helper
{
    ...
    public function appNav($appNav, $request, $user, $Html): string
    {
        ...
        foreach ($appNav as $nav) {
            if ($nav['type'] === 'link') {
                ...
                $return .= $this->appControllerNav($nav['label'], $nav['url'], $request, $Html, $user, $nav['icon'], $nav['activeUrls'], $nav['sublinks']);
                ...
                foreach ($nav['children'] as $link) {
                    ...
                    $sectionHtml .= $this->appControllerNav($link['label'], $link['url'], $request, $Html, $user, $link['icon'], $link['activeUrls'], $link['sublinks']);
                }
                if ($sectionHtml !== '')
                    $return .= $this->appControllerNavSpacer($nav['label'], $Html, $nav['icon']) . $sectionHtml;                   
            }
        }
        return $return;
    }
    public function appControllerNav($label, $url, $request, $Html, $user, $icon, $activeLinks = [], $sublinks = []): string
    {
            ...
            if($useActive){
                foreach ($sublinks as $sublink) {
                    ...
                    if ($user->canAccessUrl($suburl)) {
                        ...
                        $linkOptions['class'] = 'nav-link bi ' . $sublink['icon'] . ' ms-3 fs-6 pt-0';
                        $return .= $Html->link(__(' ' . $sublink['label']), $suburl, $linkOptions);
                    }
                }
            }
            if (!$useActive)
    ...
    protected function matchingUrl($url, $request): bool
    {
        ...
        $action = $url['action'];
        $id = $url[0] ?? null;
        if (!$request->getParam('pass') || !array_key_exists(0, $request->getParam('pass'))) {
            $request_pass = null;
        } else {
