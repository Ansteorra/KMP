...
<head>
    ...
    <link rel="manifest"
        href="<?= $this->Url->build(['controller' => 'Members', 'action' => 'card.webmanifest', $currentUser->mobile_card_token]) ?>" />
    <?php endif; ?>
...
<body class="viewMobileCard">
    <?= $this->Flash->render() ?>
    <div data-controller="member-mobile-card-pwa member-mobile-card-profile" <?php if (isset($cardUrl)): ?>
        ...
        <json data-member-mobile-card-pwa-target="urlCache">
            ...
            if ($cardUrlForManifest) {
                $cacheList[] = $this->Url->build(['controller' => 'Members', 'action' => 'viewMobileCardJson', $currentUser->mobile_card_token]);
            }
