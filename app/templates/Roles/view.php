...
<div class="roles view large-9 medium-8 columns content">
    <div class="row align-items-start">
        ...
        <div class="col text-end">
            ...
            <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $role->id], ['confirm' => __('Are you sure you want to delete {0}?', $role->name), 'title' => __('Delete'), 'class' => 'btn btn-danger btn-sm']) ?>
        </div>
    </div>
    <div class="related pt-2">
        <h4><?= __('Related Members') ?> : <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                        ...
                        <table class="table table-striped">
                            ...
                            foreach ($active as $assignee): ?>
                                <tr>
                                    ...
                                    <td><?= h($assignee->approved_by->sca_name) ?></td>
                                    <td class="actions">
...
<div class="related">
    <h4><?= __('Related Permissions') ?> : <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
            ...
            <table class="table table-striped">
                ...
                <?php foreach ($role->permissions as $permission): ?>
                    <tr>
                        ...
                        <td><?= h((($permission->authorization_type === null) ? '' : $permission->authorization_type->name)) ?>
                        </td>
                        <td class="text-center"><?= $this->Kmp->bool($permission->require_active_membership, $this->Html) ?></td>
                        <td class="text-center"><?= $this->Kmp->bool($permission->require_active_background_check, $this->Html) ?>
                        </td>
                        <td class="text-center"><?= h($permission->require_min_age) ?></td>
                        <td class="text-center"><?= $this->Kmp->bool($permission->is_super_user, $this->Html) ?></td>
                        <td class="text-center"><?= $this->Kmp->bool($permission->system, $this->Html) ?></td>
                        <td class="actions">
...
<fieldset>
    ...
    echo $this->Form->control('role_id', ['type' => 'hidden', 'value' => $role->id, 'id' => 'add_permission__role_id']);
    echo $this->Form->end()
        ?>
</fieldset>
<?php
...
<?php
echo $this->Modal->create("Edit", ['id' => 'editModal', 'close' => true]);
?>
<fieldset>
    <?php
    echo $this->Form->create($role, ['id' => 'edit_entity', 'url' => ['controller' => 'Roles', 'action' => 'edit', $role->id]]);
