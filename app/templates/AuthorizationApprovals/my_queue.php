...
?>
<h3>
    <?= h($user->sca_name) ?> Authorization Request Queue
</h3>
                ...
                <tbody>
                    <?php foreach ($denied as $request): ?>
                        <tr>
                            ...
                            <td><?= h($request->responded_on) ?></td>
                            <td><?= h($request->authorization->authorization_type->name) ?></td>
                            <td><?= h($request->authorization->approver_notes) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
...
<fieldset>
    <?php
    echo $this->Form->create(null,['url' => ['controller'=>'AuthorizationApproval', 'action' => 'deny'], 'id' => 'deny_auth']);
    echo $this->Form->control('id', ['type' => 'hidden']);
    echo $this->Form->control('approver_notes', ['type' => 'textarea', 'label' => 'Reason for Denial', 'onkeypress'=>'$("#deny_auth__submit").removeAttr("disabled");"']);
    echo $this->Form->end()
        ?>
</fieldset>
<?php
