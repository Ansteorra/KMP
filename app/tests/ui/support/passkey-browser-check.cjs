...
(async () => {
    ...
    try {
        ...
        const dialog = page.getByRole('dialog', { name: 'Sign in with a passkey', exact: true });
        ...
        await page.waitForFunction(() => document.activeElement?.textContent === 'Manage passkeys');
        await page.getByRole('button', { name: 'Manage passkeys', exact: true }).click();
        await dialog.getByRole('button', { name: 'Get started', exact: true }).click();
        ...
        await page.screenshot({ path: '/tmp/kmp-passkey-modal-mobile.png', fullPage: true });
        ...
        const revoke = page.getByRole('dialog', { name: 'Sign out all devices?', exact: true });
        await revoke.getByText('All passkeys for this account will stop working.', { exact: true }).waitFor();
        ...
        try {
            ...
            await loginAs(desk, fixture.email);
            await desk.goto('/members/profile');
            const profileUrl = desk.url();
            await desk.getByRole('button', { name: 'Manage passkeys', exact: true }).click();
            const settings = desk.getByRole('dialog', { name: 'Sign in with a passkey', exact: true });
            await settings.getByRole('button', { name: 'Get started', exact: true }).waitFor();
            assert.equal(desk.url(), profileUrl);
            await desk.screenshot({ path: '/tmp/kmp-passkey-wizard-desktop.png', fullPage: true });
            await desk.keyboard.press('Escape');
            await settings.waitFor({ state: 'hidden' });
        } finally { await desktop.close(); }
        await page.getByRole('button', { name: 'Sign out all devices', exact: true }).click();
        await revoke.getByRole('button', { name: 'Sign out all devices', exact: true }).click();
        await page.waitForURL('**/members/login');
        assert.deepEqual(errors, []);
        console.log('PASS: password-authorized passkey enrollment, real server signature verification, login after logout, removal denial, mobile and desktop modals, sign-out confirmation, focus and mobile reflow.');
    } finally {
        await browser.close();
