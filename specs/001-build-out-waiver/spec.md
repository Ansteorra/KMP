# Feature Specification: Event Waiver Tracking System
...
- Q: How should retention policies be defined and stored to enable automated expiration calculation? → A: Structured format with separate fields (number + unit + anchor): e.g., 7 years + event_end_date
...
- Q: What should happen when someone tries to modify an event's activities after waivers have been uploaded? → A: Block the change - prevent editing activities once any waivers are uploaded
- Q: How should the system handle multiple users uploading waivers for the same event simultaneously? → A: Allow all uploads - no restriction, all PDFs are accepted and added to the event

### Session 2025-10-08

- Q: Should Event Activity be a core entity or part of the Waivers plugin? → A: **Event Activity should be a CORE entity** (`src/Model/`). Event Activities (Armored Combat, Archery, Feast, Court) are reusable domain concepts that can be used by multiple plugins: Waivers plugin (track waiver requirements), Awards plugin (track which activity an award was given at), and future plugins (registrations, schedules).
...
1. **Given** I am a Kingdom officer, **When** I create a new waiver type with a name, PDF template URL, and retention policy (e.g., "7 years from event date"), **Then** the waiver type is saved and available for use in event activities
...
### User Story 2 - Configure Event Types and Activities (Priority: P2)

As a Kingdom officer, I need to define event types (e.g., "Tournament", "Practice", "Arts & Sciences") and event activities (e.g., "Armored Combat", "Archery", "Fencing") with their associated required waivers so that event stewards know which waivers must be collected.

**Why this priority**: This builds on the waiver types (P1) and establishes the event framework. It's needed before creating actual events but is secondary to defining the waiver types themselves.

**Independent Test**: Can be fully tested by creating event types, creating activities, and associating waivers with activities. Delivers value by providing a standardized catalog of event configurations.

**Acceptance Scenarios**:

1. **Given** I am a Kingdom officer, **When** I create a new event type with a name, **Then** the event type is saved and available for event creation
2. **Given** I am a Kingdom officer, **When** I create a new event activity with a name, **Then** the activity is saved
3. **Given** an event activity exists and waiver types are defined, **When** I associate waiver types with the activity, **Then** I can mark each associated waiver as required or optional
4. **Given** an event activity has associated waivers, **When** I view the activity details, **Then** I see all associated waivers with clear indication of which are required
5. **Given** I am editing an event activity, **When** I add or remove waiver associations, **Then** the changes are saved and do not affect existing events (they retain their original waiver requirements)

---

### User Story 3 - Create and Manage Events (Priority: P3)

As an event steward or branch officer, I need to create events with basic information (name, type, branch, dates, activities) and indicate whether waivers were collected so that we can track event occurrences and their waiver requirements.

**Why this priority**: This depends on event types and activities (P2) being configured. It enables event tracking but doesn't yet include waiver uploads, which is the most complex part.

**Independent Test**: Can be fully tested by creating events with various configurations, selecting activities, and marking waiver collection status. Delivers value by providing event record-keeping and waiver requirement determination.

**Acceptance Scenarios**:

1. **Given** I am an event steward, **When** I create a new event with name, event type, branch, start date, end date, and notes, **Then** the event is saved
2. **Given** I am creating an event, **When** I select event activities from the available list, **Then** the system automatically determines the required waivers based on those activities
3. **Given** an event has activities with required waivers, **When** I view the event details, **Then** I see a list of all required waivers for this event
4. **Given** I am creating an event, **When** I mark "waivers were collected" as true, **Then** the system enables waiver upload functionality for this event
5. **Given** I am creating an event, **When** I mark "waivers were collected" as false, **Then** the system does not require or allow waiver uploads
6. **Given** an event spans multiple days, **When** I provide start and end dates, **Then** the system accepts the date range and displays it correctly
7. **Given** I have appropriate permissions, **When** I edit an existing event that has no uploaded waivers, **Then** I can modify all event details including activities
8. **Given** an event has uploaded waivers, **When** I attempt to modify the event's activities, **Then** the system prevents the change and displays a message explaining that activities are locked once waivers are uploaded

---

### User Story 4 - Upload and Manage Event Waivers (Priority: P4)

As an event steward, I need to upload signed waiver PDFs for events where waivers were collected so that we maintain legal documentation and comply with retention policies.
...
**Independent Test**: Can be fully tested by uploading waivers to events, viewing uploaded waivers, and verifying retention policies are captured. Delivers complete waiver management capability.
...
### User Story 5 - Search and Report on Event Waivers (Priority: P5)

As a Kingdom officer or legal compliance officer, I need to search for events and their associated waivers and generate reports so that I can respond to legal inquiries and manage document retention.
...
**Independent Test**: Can be fully tested by searching events by various criteria and generating reports. Delivers value through improved accessibility and compliance management.
...
- **FR-003**: Retention policy MUST use structured format with three components: duration number (integer), duration unit (years/months/days), and anchor point (event_end_date/upload_date/permanent). Examples: "7 years from event_end_date", "5 years from upload_date", "permanent"
- **FR-004**: System MUST prevent deletion of waiver types that are referenced by event activities or have uploaded waivers
...
- **FR-006**: System MUST allow authorized users to create, edit, and view event types
- **FR-007**: Each event type MUST have a unique name
- **FR-008**: System MUST prevent deletion of event types that are referenced by existing events

**Event Activity Management**:
- **FR-009**: System MUST allow authorized users to create, edit, and view event activities
- **FR-010**: Each event activity MUST have a unique name
- **FR-011**: System MUST allow associating zero or more waiver types with each event activity
- **FR-012**: For each waiver type associated with an activity, system MUST allow marking it as required or optional
- **FR-013**: System MUST prevent deletion of event activities that are referenced by existing events
- **FR-014**: When an event activity's waiver associations are modified, existing events MUST retain their original waiver requirements

**Event Management**:
- **FR-015**: System MUST allow authorized users to create, edit, and view events
- **FR-016**: Each event MUST have a name, event type, branch, start date, end date, notes field, and "waivers collected" boolean
- **FR-017**: System MUST allow associating multiple event activities with each event
- **FR-017a**: System MUST prevent editing an event's associated activities once any waivers have been uploaded for that event
- **FR-018**: System MUST automatically compile the list of required waivers based on selected event activities
- **FR-019**: System MUST support single-day and multi-day events (start date may equal end date)
- **FR-020**: System MUST display the complete list of required waivers for each event where waivers were collected
...
- **FR-029**: System MUST calculate waiver retention expiration dates based on the captured retention policy's structured format (duration + unit + anchor) and the relevant date (event end date or upload date)
...
- **FR-033**: System MUST allow searching events by date range, branch, event type, and waiver collection status
- **FR-034**: System MUST allow filtering events to show only those where waivers were collected
- **FR-035**: System MUST allow generating reports of waivers eligible for deletion based on retention policy expiration
- **FR-036**: System MUST track and display waiver collection compliance metrics (percentage of events with waivers collected and uploaded)

**Security and Permissions**:
- **FR-037**: System MUST enforce role-based access control for waiver configuration, event creation, and waiver uploads
- **FR-038**: System MUST log all waiver uploads with user identity, timestamp, and event association
...
- **Event Type**: Simple classification of events (e.g., "Tournament", "Practice", "Arts & Sciences Day", "Feast"). Contains: unique name, description, active status. Related to Events (one-to-many) and Event Activities (one-to-many). **Used by**: Awards plugin, Waivers plugin, future plugins.

- **Event**: A specific occurrence of an SCA event. Contains: name, event type reference (FK), branch reference (FK), start date, end date, location, notes. Related to Event Type (many-to-one), Branch (many-to-one). **Note**: Replaces/consolidates award_events from Awards plugin. **Used by**: Awards plugin (track where awards were given), Waivers plugin (track waiver uploads for events), future plugins.

- **Event Activity**: Specific activities that occur at events (e.g., "Armored Combat", "Rapier Combat", "Archery", "Feast", "Court"). Contains: unique name, description, event_type_id (FK), active status. Related to Event Type (many-to-one). **Used by**: Waivers plugin (determine waiver requirements), Awards plugin (track which activity an award was given at), future plugins (registrations, schedules).

#### Waivers Plugin Entities

Located in `plugins/Waivers/src/Model/Entity/` and `plugins/Waivers/src/Model/Table/`:

- **Waiver Type**: Represents a category of legal waiver document (e.g., "Adult Combat Waiver", "Minor Waiver", "Archery Waiver"). Contains: unique name, description, PDF template reference, current retention policy (structured: duration + unit + anchor), active status. Related to Event Activity Waivers (one-to-many) and Event Waivers (one-to-many).
...
3. **Single Branch Per Event**: Each event is run by one primary branch, though the event may involve participants from multiple branches.
...
6. **Batch Uploads**: Event stewards will upload waiver PDFs one at a time or in small batches through a web interface. Large-scale bulk imports are not required for initial release.

7. **Concurrent Uploads**: Multiple users may upload waivers to the same event simultaneously. The system handles concurrent uploads without locking or queuing; all uploads are accepted and stored.

8. **Member Signature Tracking**: Individual member signatures within waiver PDFs are not tracked or indexed. Waivers are tracked at the document level per event, not per member per waiver.
...
11. **Event Immutability After Upload**: Once waivers are uploaded for an event, the event's activity associations become immutable to preserve data integrity and prevent orphaned waivers.

12. **Core vs Plugin Architecture**: Event, Event Type, and Event Activity are core KMP entities available to all plugins. Waiver-specific functionality (Waiver Type, Event Activity Waiver, Event Waiver) resides in the Waivers plugin.

13. **Awards Plugin Migration**: Existing award_events data will be migrated to the core Event entity. The Awards plugin will be refactored to use core Event and optionally Event Activity (to track which activity an award was given at), eliminating the award_events table.
...
- **Event Type**: Classification system for all events across the kingdom (Tournament, Feast, Workshop, Practice)
- **Event**: Represents any SCA event occurrence (tournaments, practices, meetings, etc.)
- **Event Activity**: Specific activities that happen at events (Armored Combat, Archery, Feast, Court, etc.)
...
- **Event Activity Waiver**: Junction entity linking Event Activities (core) to required Waiver Types
- **Event Waiver**: Actual waiver PDFs uploaded for events
- This plugin references core entities via foreign keys but keeps waiver-specific logic isolated

**Data Flow**:
- **Configuration**: Event Type (core) → Event Activities (core) → Event Activity Waivers (plugin)
  - Event Types define categories
  - Event Activities define what happens at events
  - Event Activity Waivers define which waiver types are required for each activity
- **Waiver Upload**: Events (core) → Event Waivers (plugin)
  - Events are specific instances
  - Event Waivers are the actual uploaded files for those events

**Rationale**: 
- Events and activities are fundamental SCA concepts that multiple systems need to track
- Event activities are reusable domain concepts (combat, archery, feast) independent of waivers
- Waivers are a specialized legal/compliance concern best isolated in a plugin
- This separation allows the Awards plugin to track "Best Fighter at Armored Combat activity" and Waivers plugin to track "Combat Waiver required for Armored Combat activity" using the same Event Activity entity
- Clear plugin boundaries enable independent testing and optional deployment

### Migration Strategy

**Phase 1: Core Event System**
1. Create Event Type, Event, and Event Activity entities in core KMP (`src/Model/`)
2. Event Activities can be associated with Event Types (defining typical activities for event categories)
3. Migrate data from Awards plugin's award_events table to new core Event
4. Maintain backward compatibility during transition
5. Test Event and Activity CRUD operations independently
...
3. Implement Event Activity Waiver junction entity (links core Event Activities to required Waiver Types)
4. Implement Event Waiver entity (uploaded files linked to core Events)
5. Create foreign key relationships to core Event and Event Activity entities
6. Test Waivers plugin functionality

**Phase 3: Awards Plugin Refactoring**
1. Refactor Awards plugin to reference core Event and Event Activity entities
2. Optionally link awards to Event Activities (e.g., "Best Fighter" at "Armored Combat" activity)
3. Remove award_events table (data already migrated)
4. Update Awards plugin queries, relationships, and tests

**Phase 4: Integration Testing**
1. Verify Awards plugin functionality with core Events and Activities
2. Test Waivers plugin integration with core Events and Activities
...
4. **Automated Waiver Reminders**: No automatic notifications to event stewards about missing waivers.
5. **Waiver Statistics Per Member**: No tracking of which members have signed which waivers across multiple events.
